...
def _render_horizontal_subquestions(doc, questions: list, layout: str = 'horizontal', temp_dir: str = None, sec_title: str = ''):
    ...
    if is_two_col:
        ...
        prevent_row_split(sub_tbl)
        set_no_borders(sub_tbl)

        col_width = Inches(PAGE_CONTENT_WIDTH_INCHES / 2)
        ...
        col_width = Inches(PAGE_CONTENT_WIDTH_INCHES / len(questions))
        for col in sub_tbl.columns:
            col.width = col_width
        for q_idx, q in enumerate(questions):
...
def _render_mcq_question(doc, q: dict, q_text: str, q_idx: int = 0, default_layout: str = 'horizontal'):
    ...
    if is_two_col:
        ...
        t_opt.autofit = False
        prevent_row_split(t_opt)
        set_no_borders(t_opt)

        col_w_opt = Inches(PAGE_CONTENT_WIDTH_INCHES / 2)
        ...
        for i, opt in enumerate(opts):
            r_idx = i // 2
            c_idx = i % 2
            cell = t_opt.rows[r_idx].cells[c_idx]
            cell.width = col_w_opt
            set_cell_margins(cell, top=10, bottom=14, left=15, right=15)
            p_opt = cell.paragraphs[0]
            opt_after = DOCX_QUESTION_SPACE_AFTER if r_idx == num_rows - 1 else Pt(3)
        ...
        col_w_opt = Inches(PAGE_CONTENT_WIDTH_INCHES / len(opts))
        for col in t_opt.columns:
            col.width = col_w_opt
        for i, opt in enumerate(opts):
            c = t_opt.rows[0].cells[i]
            c.width = col_w_opt
            set_cell_margins(c, top=20, bottom=25, left=15, right=15)
            p_opt = c.paragraphs[0]
            fmt_paragraph(p_opt, before=Pt(0), after=DOCX_QUESTION_SPACE_AFTER, spacing=1.0)
            add_formatted_math_text(p_opt, opt, font_size=DOCX_BODY_FONT_SIZE)
